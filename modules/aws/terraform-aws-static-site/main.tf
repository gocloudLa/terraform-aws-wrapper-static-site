module "cloudfront" {
  ...
  origin_access_control = {
    "${var.bucket}-origin-access-control" = {
      ...
      name             = try(var.origin_access_control_name, "${var.bucket}-origin-access-control")
      origin_type      = "s3"
  ...
  origin = {
    "${var.bucket}" = {
      ...
      origin_access_control_key = try(var.origin_access_control_key_name, "${var.bucket}-origin-access-control")
    }
  }
